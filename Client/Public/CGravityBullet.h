...
BEGIN(Client)
class CGravityBullet final : public CBullet
{
public:
...
public:
	typedef struct CGravityBULLET_DESC : public CBullet::CBullet_DESC
	{
...
private:
	explicit CGravityBullet(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	explicit CGravityBullet(const CGravityBullet& rhs);
	virtual ~CGravityBullet() = default;
...
public:
	static CGravityBullet* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
