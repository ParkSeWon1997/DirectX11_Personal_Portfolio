...
PS_OUT PS_Blink(PS_IN In)
{
    ...
    float fBlinkFrequency = pow((g_MaxTime - g_fTime) / g_MaxTime, 2);
    float fBlink = abs(sin(g_fTime * 10.0f) * fBlinkFrequency);
    vDiffuse.rgb = lerp(vDiffuse.rgb, float3(1.0f, 0.0f, 0.0f), fBlink);
