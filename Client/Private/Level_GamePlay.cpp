...
HRESULT CLevel_GamePlay::Ready_UI(const wstring& strLayerTag)
{
	...
	//desc.strModelName = TEXT("Icon_Keyboard_Space");
	//desc.fSizeX = 64.f * 0.5f;
	//desc.fSizeY = 64.f * 0.5f;
	//desc.fX = (g_iWinSizeX * 0.6f) + desc.fSizeX + 375.f;
...
HRESULT CLevel_GamePlay::Ready_Player_UI(const wstring& strLayerTag)
{
	UI::UI_DESC desc;
	...
	desc.iPassIndex = 4;
	if (FAILED(m_pGameInstance->Add_CloneObject(LEVEL_GAMEPLAY, strLayerTag, TEXT("Prototype_GameObject_UI_PlayerIcon"), &desc)))
		return E_FAIL;
...
HRESULT CLevel_GamePlay::Ready_Player_SKill_UI(const wstring& strLayerTag)
{
	UI::UI_DESC desc;
	...
	desc.fY = 580.f;
	desc.iPassIndex = 6;
	if (FAILED(m_pGameInstance->Add_CloneObject(LEVEL_GAMEPLAY, strLayerTag, TEXT("Prototype_GameObject_UI_PlayerSkill"), &desc)))
		return E_FAIL;
...
HRESULT CLevel_GamePlay::Ready_Layer_Environment(const wstring& strLayerTag)
{
	...
	if (FAILED(m_pGameInstance->Add_CloneObject(LEVEL_GAMEPLAY, strLayerTag, TEXT("Prototype_ItemBox"), &ItemDesc)))
		return E_FAIL;
