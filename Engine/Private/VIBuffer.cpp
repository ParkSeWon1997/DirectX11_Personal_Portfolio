...
CVIBuffer::CVIBuffer(const CVIBuffer & rhs)
	...
	, m_MeshBufferDesc(rhs.m_MeshBufferDesc)
...
HRESULT CVIBuffer::Create_Buffer_For_Mesh(ID3D11Buffer** ppVIBuffer)
{

	if (FAILED(m_pDevice->CreateBuffer(&m_MeshBufferDesc, &m_InitialData, ppVIBuffer)))
		return E_FAIL;
	return S_OK;
}
...
void CVIBuffer::Free()
{
	...
	if (false == m_isCloned)
		Safe_Delete(m_pVertexPositions);
